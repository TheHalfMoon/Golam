...
|---|---|---|---|
| SQLite | `rusqlite 0.40.2` | ADMITTED_THIS_SLICE | MIT wrapper; `libsqlite3-sys` is an explicit FFI boundary. `bundled` is enabled for deterministic cross-platform SQLite 3.53.2; vendored SQLite is public domain. All DB authority semantics remain in `golam-ledger`. |
| Integrity hash | `blake3 1.8.7` | ADMITTED | Official BLAKE3 implementation. Exact pin with `pure` for the early spine avoids assembly/C optimized paths while canonical vectors are frozen. Revisit performance only after correctness/security qualification. |
...
The first SQLite slice intentionally proves only connection configuration, forward migration, required-table presence, future-schema refusal, and `PRAGMA quick_check`. Event/effect writes, transaction ordering, checkpoint replay, disk-full recovery, and hash-chain startup verification remain separate tasks and are not implied by successful migration tests.
