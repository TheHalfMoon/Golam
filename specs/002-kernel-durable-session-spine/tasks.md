...
**Last reconciled proven code head**: `da40376f16a6dd1eee45b02b1110f3f1ffda1a45`  
**Exact-head CI evidence**: run `32949212652` / run number `191` — Windows, macOS, Linux `fmt + clippy -D warnings + test` PASS
...
- [x] **T002-003** Add baseline CI for fmt/clippy/test on Windows/macOS/Linux; do not claim green until runs exist. — PASS; exact-head matrix runs exist, latest proven code run `32949212652`.
...
- [x] **T002-054** Build fault injector for every transition and simulated remote accept/ack boundary. — PASS at `5f0e773297984e63c6e3e8402c40fa9d50`? NO. (continued below)
...
- [x] **T002-061** Evaluate/implement preallocated disk recovery reserve; prove or remove the guarantee based on tests. — PASS at `da40376f16a6dd1eee45b02b1110f3f1ffda1a45`, CI `32949212652`: decision is `NO_RECOVERY_RESERVE_GUARANTEE`; `implementation/recovery-reserve-evaluation.md` removes the unproven reserve-backed durability claim, and `recovery_reserve_policy.rs` proves real startup does not create or depend on `recovery-reserve.bin` on Windows/macOS/Linux. Disk-full pre-intent remains fail-closed; post-dispatch failure remains reconciliation/manual-review territory.
- [x] **T002-062** Implement minimal CLI commands for client enroll, sessions, replay/fork/checkpoint, effect simulator/reconcile and doctor/recovery report. — PASS at `da40376f16a6dd1eee45b02b1110f3f1ffda1a45`, CI `32949212652`: the Rust `golam` CLI parses the bounded command surface and executes through authenticated OS-local IPC; `golamd` performs `HELLO -> CHALLENGE -> AUTHENTICATE -> READY`, explicit first-client bootstrap approval, request/reply settlement and `CommandRouter -> KernelApi` dispatch. No TCP/HTTP control path or direct privileged-ledger dependency was introduced; Windows/macOS/Linux matrix tests pass.
