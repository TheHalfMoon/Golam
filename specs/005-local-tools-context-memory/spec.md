...
- every protected memory mutation intent binds the exact dedicated memory-operational-SQLite store identity/schema in addition to expected Markdown identity/digest/version, authorization/promotion evidence and Effect identity, and those bindings survive PREPARED unchanged;
...
- Agent Skills-compatible `SKILL.md` instruction packages with provenance, immutable version/content digest, reviewed local capability mapping and lifecycle state;
- executable skill scripts remain disabled until the owning production sandbox profile is qualified;
- immediately before every instruction activation or executable dispatch, the exact reviewed skill package/version/content digest/capability mapping and current lifecycle state are revalidated; `DEPRECATED`/`REVOKED`/replaced/unknown/mismatched state invalidates queued, prepared-but-not-dispatched, cached capability/approval and dispatch-decision state;
...
- every MCP dispatch revalidates the exact reviewed binding identity/digest, version lock and Golam-local mapping identity/digest immediately before local or remote dispatch; stale/replaced/revoked/mismatched state invalidates queued/prepared calls, mapped descriptors, cached capability/approval material and dispatch decisions;
...
- **FR-013** Every Golam-generated managed-memory mutation MUST use an immutable `MemoryMutationIntent` bound to the initiating principal, current Kernel authorization, applicable promotion approval/pre-registered verifier evidence, expected current versions, exact expected observed Markdown target identity/digest/version, exact dedicated memory-operational-SQLite store identity/schema, and a unique effect identity. All protected bindings MUST participate in the mutation-intent digest and survive durable PREPARED unchanged. The authorized Effect Gate intent MUST be durably PREPARED in the existing protected authority database before the first canonical Markdown or memory-operational-SQLite mutation; the dedicated memory operational SQLite store remains separate and MUST bind the exact effect identity and intent digest. No cross-store atomicity may be assumed. Only the single governed memory writer/handler may execute the prepared mutation. Markdown replacement MUST revalidate the expected digest/version and target identity at the commit boundary and fail closed as `USER_EDIT_DETECTED`/`CONFLICT` when they changed or cannot be preserved. Integrity-chained terminal outcome plus required read-back/verification evidence across the authority journal, exact Markdown identity/digest/version and exact memory-operational-SQLite store/effect rows MUST be recorded. Crash, disconnect, wrong-store binding or cross-store disagreement MUST remain failed/reconciling or `UNKNOWN_OUTCOME` and block dependent managed-memory mutations until reconciliation. This lifecycle applies to all operations, including multi-store `FORGET` and `REDACT`.
...
- **FR-019** Instruction-only skills MAY be admitted after provenance/capability review; executable scripts require qualified sandbox containment. Every activation/dispatch decision MUST bind the exact reviewed skill package/version/content digest and reviewed local capability mapping. Immediately before instruction activation or executable dispatch, Golam MUST re-read lifecycle state and revalidate that exact binding; deprecated/revoked/replaced/unknown/version/digest/mapping-mismatched state MUST invalidate queued calls, prepared-but-not-dispatched calls, cached capability/approval material and cached dispatch decisions rather than replaying prior authority.
- **FR-020** MCP server output MUST be treated as untrusted/tainted input and server-advertised tools/resources MUST NOT mint Golam authority. Every local or remote MCP dispatch MUST bind and immediately revalidate the exact active reviewed binding identity/digest, version lock and Golam-local mapping identity/digest. Deprecated/revoked/replaced/unreviewed/unknown/version/digest/mapping-mismatched state MUST reject queued calls, prepared-but-not-dispatched calls, stale mapped descriptors, cached capability/approval material and cached dispatch decisions; a superseded binding cannot donate authority to its replacement.
...
- **SC-006** Managed-memory mutations prove authority-database PREPARED-before-mutation evidence, exact intent target/digest/version/store bindings, conditional Markdown commit-time precondition enforcement, memory-SQLite/effect binding, terminal cross-store verification and `UNKNOWN_OUTCOME` reconciliation; FORGET/REDACT removes active canonical knowledge and rebuilds every enabled derivative deterministically.
- **SC-007** Malicious skill/MCP/memory inputs cannot mint authority, clear taint or bypass Effect Gate, and stale/revoked/replaced/mismatched exact skill/MCP dispatch bindings cannot dispatch queued/prepared/cached/approved calls or decisions.
