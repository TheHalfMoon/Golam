...
- [ ] **T196 — Voice Presence and Audio Authority Contract.** Refine T140/T169/T175 using VoiceStudio as a product/engine reference. Require explicit short-lived microphone authority, visible listening/recording state, immediate revoke/stop, push-to-talk or explicit start by default, local-first STT/TTS, barge-in, ephemeral raw audio default, transcript provenance/taint, engine/model artifact identity and explicit cloud capability when used. Treat voice cloning/speaker imitation as a separate bounded capability with source provenance, explicit consent/authorization policy and no authentication role; generated voice may never satisfy owner-presence or approval requirements. `VOICE_CONTENT != AUTHENTICATION`; `TRANSCRIPT != APPROVAL`; `SYNTHETIC_VOICE != OWNER_PRESENCE`.
