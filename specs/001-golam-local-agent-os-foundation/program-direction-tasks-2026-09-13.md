...
- [ ] **T196 — Voice Presence and Audio Authority Contract.** Refine T140/T169/T175 using VoiceStudio as a product/engine reference. Require explicit short-lived microphone authority, visible listening/recording state, immediate revoke/stop, push-to-talk or explicit start by default, local-first STT/TTS, barge-in, ephemeral raw audio default, transcript provenance/taint, engine/model artifact identity and explicit cloud capability when used. `VOICE_CONTENT != AUTHENTICATION`; `TRANSCRIPT != APPROVAL`.

- [ ] **T197 — Donor Firewall and TCB Budget.** Add a machine-readable source-admission posture by architectural plane plus a privileged TCB budget. Track privileged dependency growth, unsafe/FFI/native parser surfaces, filesystem/network/device privileges, secret residency, process-spawn ability and exceptional donor-code admissions. Prefer ports/reimplementation in Authority/Evidence, bounded adapters in Capability/Context and broader selective reuse in Harness/Experience only after Source Foundry. A source permission grant never exempts the TCB budget.

- [ ] **T198 — Spec 009 Bounded-Package Decomposition.** Keep Spec 009 as an umbrella program target but prohibit one monolithic implementation authority package. Before any Spec 009 implementation authorization, produce an ownership/dependency map for independently reviewable bounded packages covering at minimum: Control Route Fabric + Browser/Application Semantics; Knowledge Ingestion + Research Evidence; Document/Artifact Semantics; Agent Experience Projection + Task/Routine UX; Voice + Presence; and Context Compiler/Repository Intelligence. Exact spec numbers/order are assigned only after live successor-authority verification.

## Phase Q — Capability ontology and product convergence

- [ ] **T199 — Unified Operation/Effect Ontology.** Define one typed operation/effect classification used across native desktop, browser, filesystem, process, Git, document, MCP, connector, research, voice, mobile and worker surfaces. The ontology must express read/observe vs mutate, externality, reversibility, idempotency/retry class, target identity strength, secret/taint/egress implications, required visibility, verification obligations and approval policy inputs. Adapters map into this ontology; they do not define parallel authority semantics.
...
- [ ] **T201 — Privacy Profile Contract.** Make `STRICT_LOCAL`, `LOCAL_PLUS_APPROVED_CONNECTORS`, and `HYBRID_MANAGED` (names provisional) explicit top-level privacy profiles derived into capability/egress/model/execution policy. Profiles may narrow or preconfigure allowed capabilities but cannot bypass per-effect authority or secret policy. Switching to a more permissive profile is itself a governed configuration effect with clear data-path disclosure.
...
P0_NEW:
  T197 Donor Firewall / TCB Budget
  T198 Spec 009 decomposition
  T199 Unified Operation/Effect Ontology
...
- T185 depends on T167 and T183 and must consume projections rather than create a new authority/event database.
...
- T190 depends on T151 isolation and T199 operation/effect classes.
...
- T197 is a governance/source-admission prerequisite for broad donor reuse.
- T198 is an implementation-scope prerequisite before any Spec 009 umbrella authorization.
...
- T201 refines strict-local/egress policy; strict local remains fail closed.
- T202 consumes T159 SLO/resource evidence and T197 TCB/admission evidence.
...
6. Decompose future capability work into bounded independently reviewable units; never treat this task file as direct implementation permission.
