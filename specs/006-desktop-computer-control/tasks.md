...
- [ ] **T006-P009** Run exact-head CI on the complete repaired planning branch. All earlier CI evidence becomes historical after any planning-head mutation and cannot satisfy this gate.
...
- [ ] **T006-003** Add pure prepared semantic-action, focus, raw-fallback, capture and clipboard intents with distinct immutable request/effect bindings, canonical request/intent digests, exact fallback-eligibility evidence for weaker routes, exact control-lease generation where applicable and distinct authority classes.
...
- [ ] **T006-005** Add fake-backend contract tests for constitutional route ordering, stronger-route availability, unreconciled `UNKNOWN_OUTCOME`, observation bounds, permission loss, focus race, stale target, semantic action, governed focus, capture, pixel-hint staleness/non-authority, raw fallback, clipboard denial, superseded control-lease generation and loss of the qualified visible-control channel; reject missing, stale, mismatched or substituted request/effect/authority/fallback-eligibility bindings and Kernel/Effect Gate authorization before focus, raw, clipboard or other side-effect dispatch.

## Phase B — Authority, effect, route, interrupt and evidence lifecycle

- [ ] **T006-006** Add Kernel route evaluation plus prepare/revalidate/dispatch/finalize lifecycle for semantic actions and focus with immutable ToolRequest/effect/intent bindings; every consequential dispatch traverses `Effect PREPARED → Kernel/Effect Gate → immediate revalidation`, selects the highest applicable authorized constitutional route, and binds exact current control-lease generation and required visible-control-channel state for interactive effects.
- [ ] **T006-007** Add distinct Kernel/Effect lifecycle for raw input fallback plus protected control-lease/input-authority state, qualified visible-control-channel state, and immediate human `PAUSE`/`STOP`/`TAKEOVER`/release transitions. Raw fallback requires canonical fresh fallback-eligibility evidence and explicit policy/approval, then traverses `Effect PREPARED → Kernel/Effect Gate → immediate route/target/session/permission/lease/visible-channel revalidation → bounded dispatch → terminal evidence/reconciliation`; missing, stale, mismatched or substituted Gate authorization fails closed. Raw fallback cannot inherit semantic/capture/pixel authority. New autonomous input fails closed while a stronger applicable route remains eligible, an `UNKNOWN_OUTCOME` is unresolved, or no qualified visible control channel is active.
...
- [ ] **T006-009** Add explicit clipboard read/write lifecycle with immutable request/effect/intent bindings, capability/policy/approval, `Effect PREPARED`, `Kernel/Effect Gate`, immediate binding/session/permission revalidation, one bounded operation and terminal evidence/reconciliation; reject missing, stale, mismatched or substituted Gate authorization and deny silent/background inspection.
...
- [ ] **T006-020** Implement selected-source capture contract and resource bounds through the governed `ToolRequest → capability/policy/approval → Effect PREPARED → Kernel/Effect Gate → immediate revalidation → bounded dispatch → terminal/reconciliation` lifecycle.
...
- [ ] **T006-025** After exact Source Foundry qualification for any introduced Windows binding/library, implement Windows UI Automation semantic action and focus adapters with valid current Kernel/Effect Gate authorization, target/locked/UAC/secure-desktop/session-transition revalidation, and focus readback tests.
- [ ] **T006-026** After exact Source Foundry qualification for any introduced macOS binding/library, implement AXUIElement semantic action and focus adapters with valid current Kernel/Effect Gate authorization plus permission/stale-element/TCC/readback tests.
- [ ] **T006-027** After exact Source Foundry qualification for any introduced Linux binding/library, implement AT-SPI semantic action and focus adapters with valid current Kernel/Effect Gate authorization plus accessibility-service/session/readback tests.
- [ ] **T006-028** Add post-action verification and outcome evidence across supported semantic action and focus families, including route disposition, interrupted/takeover outcomes and durable focus `UNKNOWN_OUTCOME` reconciliation.

## Phase G — Explicit raw fallback, bounded pixel hint and clipboard

- [ ] **T006-029** After exact Source Foundry qualification for any introduced Windows binding/library, implement explicit `SendInput` fallback behind canonical fallback-eligibility evidence plus separate capability/policy/approval/effect/control-lease generation; require valid current Kernel/Effect Gate authorization before dispatch and fail closed on absent/stale/mismatched/substituted Gate state; permanently deny locked/UAC/secure desktop control.
- [ ] **T006-030** After exact Source Foundry qualification for any introduced macOS binding/library, implement explicit raw input fallback only after route eligibility is established and OS permission/policy permit it; require `Effect PREPARED → Kernel/Effect Gate → immediate revalidation` before dispatch; no background capture/keylogging.
- [ ] **T006-031** After exact Source Foundry qualification for any introduced Linux binding/library, implement raw input only after route eligibility is established, for explicit X11 sessions or compositor/user-granted Wayland RemoteDesktop/EIS path; require `Effect PREPARED → Kernel/Effect Gate → immediate revalidation` before dispatch; no bypass.
- [ ] **T006-032** After exact Source Foundry qualification for any introduced clipboard binding/library, implement explicit clipboard read/write adapters with immutable request/effect/intent bindings and capability/policy/approval, traversing `Effect PREPARED → Kernel/Effect Gate → immediate revalidation → one bounded operation → terminal evidence/reconciliation`; reject absent, stale, mismatched or substituted Gate authorization and deny polling.
- [ ] **T006-033** Implement bounded `PixelTargetHint` plumbing from explicitly authorized capture to raw fallback as untrusted geometry only; no OCR/text extraction, no authority, fallback-eligibility or Gate-authorization minting, and no stale-source reuse. Pixel-assisted raw fallback must traverse the same `Effect PREPARED → Kernel/Effect Gate → immediate revalidation` boundary as every raw fallback. Add adversarial tests proving semantic failure, capture success or pixel hints cannot silently escalate to raw input/clipboard authority, cannot bypass absent/stale/mismatched/substituted Gate authorization, and human takeover blocks stale input generations.
...
- [ ] **T006-038** Add stronger-route-still-available, stale/missing fallback eligibility, unreconciled `UNKNOWN_OUTCOME`, absent/stale/mismatched/substituted Kernel/Effect Gate authorization for focus/raw fallback/clipboard, stale window reuse, process restart, focus theft, wrong-window substitution, permission revocation, Windows locked/UAC/secure-desktop/session transitions, capture permission loss after prepare, uncertain focus/capture completion, portal-session termination, target substitution, visible-control-channel loss, human pause/stop/takeover, stale lease generation and takeover-latency adversarial tests; prove Gate-invalid focus/raw/clipboard dispatch is rejected, focus-dependent actions remain blocked while focus outcome is uncertain, weaker escalation/invisible autonomous actuation is suspended, and `UNKNOWN_OUTCOME` blocks conflicting retry until reconciliation.
...
- [ ] **T006-040** E2E: authenticated Tauri native host enrolls/authenticates to `golamd` without exposing auth material to renderer → persistent visible autonomous-control indicator is active → route evaluation selects highest applicable authorized route → observe exact work surface → prepare and Gate-authorize bounded focus → revalidate → dispatch focus → re-observe intended focus → choose semantic target → authorize semantic action → deterministic post-action evidence.
- [ ] **T006-041** E2E: explicitly select window/display → create bound capture request/effect → bounded capture through the Kernel/Effect Gate → ephemeral local consumption → optional bounded pixel hint remains non-authoritative/non-eligibility-bearing → terminal evidence → release resources.
- [ ] **T006-042** E2E: stronger routes unavailable/inapplicable/denied/safely failed → canonical fallback eligibility permits deterministic raw path → raw fallback remains denied by default → optional bounded pixel hint cannot authorize action or Gate dispatch → explicitly authorized matching fallback succeeds only after valid current Kernel/Effect Gate authorization plus fresh target/focus/session/control-lease/visible-channel revalidation.
- [ ] **T006-043** E2E: stronger route becomes available, `UNKNOWN_OUTCOME`, missing/stale/mismatched/substituted Gate authorization for focus/raw/clipboard, or permission/focus/identity/binding/control-lease drift between prepare and execute all fail closed without weaker escalation; uncertain focus blocks focus-dependent action until reconciliation; loss of the qualified visible-control channel suspends new actuation; human takeover invalidates queued/prepared stale input within measured bound; uncertain completion survives restart/reconnect and blocks conflicting retry until reconciliation.
