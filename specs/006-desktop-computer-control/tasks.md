...
- [ ] **T006-001** Add versioned desktop-control core types, access modes, limits, capability discovery, protected control-lease state, bounded `PixelTargetHint`, human interrupt evidence and canonical encodings/digests.
- [ ] **T006-002** Add opaque work-surface and semantic-element identities with stale/substitution validation.
- [ ] **T006-003** Add pure prepared semantic-action, raw-fallback, capture and clipboard intents with distinct immutable request/effect bindings, canonical request/intent digests, exact control-lease generation where applicable and distinct authority classes.
- [ ] **T006-004** Add a platform-neutral `DesktopBackend` trait and deterministic fake backend; keep protected lease/input-authority mutation above untrusted adapters.
- [ ] **T006-005** Add fake-backend contract tests for observation bounds, permission loss, focus race, stale target, semantic action, capture, pixel-hint staleness/non-authority, raw fallback, clipboard denial and superseded control-lease generation; reject missing, mismatched, stale or substituted request/effect/authority bindings before dispatch.

## Phase B — Authority, effect, interrupt and evidence lifecycle

- [ ] **T006-006** Add Kernel prepare/revalidate/dispatch/finalize lifecycle for semantic actions with immutable ToolRequest/effect/intent bindings and exact current control-lease generation for interactive effects.
- [ ] **T006-007** Add distinct Kernel/Effect lifecycle for raw input fallback plus protected control-lease/input-authority state and immediate human `PAUSE`/`STOP`/`TAKEOVER`/release transitions; raw fallback requires explicit policy/approval and cannot inherit semantic/capture/pixel authority.
...
- [ ] **T006-010** Add durable secret-safe desktop-control and human-interrupt evidence, request/effect/intent/control-lease digests, terminal status including `UNKNOWN_OUTCOME`, takeover latency fields and reconciliation metadata.
- [ ] **T006-011** Add restart/reconnect reconciliation for nonterminal desktop effects/sessions and control leases without widening authority; conflicting retries remain blocked until terminal truth is established and stale lease generations cannot restore agent input after human takeover.
...
- [ ] **T006-015** Add frontend authentication-disconnected, permission, unsupported, stale, paused/takeover and unknown-outcome UX states with no authority interpretation in the webview; renderer-only state cannot create or clear takeover.
...
- [ ] **T006-019** Add cross-platform fake and adapter tests proving observation, semantic text, coordinates, screenshots and pixel hints never imply actuation authority.
...
- [ ] **T006-028** Add post-action verification and outcome evidence across supported semantic action families, including interrupted/takeover outcomes.

## Phase G — Explicit raw fallback, bounded pixel hint and clipboard

- [ ] **T006-029** After exact Source Foundry qualification for any introduced Windows binding/library, implement explicit `SendInput` fallback behind separate capability/policy/approval/effect/control-lease generation; permanently deny locked/UAC/secure desktop control.
- [ ] **T006-030** After exact Source Foundry qualification for any introduced macOS binding/library, implement explicit raw input fallback only where OS permission and policy permit; no background capture/keylogging.
- [ ] **T006-031** After exact Source Foundry qualification for any introduced Linux binding/library, implement raw input for explicit X11 sessions only or compositor/user-granted Wayland RemoteDesktop/EIS path; no bypass.
...
- [ ] **T006-035** Prove no OCR/text extraction from raw screenshots and no screenshot/pixel-hint-derived authority before Spec 007.
- [ ] **T006-036** Prove no hidden network/remote/cloud fallback or HTTP dependency in desktop-control execution.
- [ ] **T006-037** Add malicious titles/accessibility text/clipboard/screenshot metadata/pixel-hint provenance tests as untrusted data.
- [ ] **T006-038** Add stale window reuse, process restart, focus theft, wrong-window substitution, permission revocation, Windows locked/UAC/secure-desktop/session transitions, capture permission loss after prepare, uncertain completion, portal-session termination, target substitution, human pause/stop/takeover, stale lease generation and takeover-latency adversarial tests; prove `UNKNOWN_OUTCOME` blocks conflicting retry until reconciliation.
...
- [ ] **T006-040** E2E: authenticated Tauri native host enrolls/authenticates to `golamd` without exposing auth material to renderer → observe surface → choose semantic target → authorize → semantic action → deterministic post-action evidence.
- [ ] **T006-041** E2E: explicitly select window/display → create bound capture request/effect → bounded capture → ephemeral local consumption → optional bounded pixel hint remains non-authoritative → terminal evidence → release resources.
- [ ] **T006-042** E2E: semantic action unavailable → raw fallback denied by default → optional bounded pixel hint cannot authorize action → explicitly authorized matching fallback succeeds only under fresh target/focus/session/control-lease authority.
- [ ] **T006-043** E2E: permission/focus/identity/binding/control-lease drift between prepare and execute fails closed without side effect; human takeover invalidates queued/prepared stale input within measured bound; uncertain completion survives restart/reconnect and blocks conflicting retry until reconciliation.

## Phase J — Final convergence

- [ ] **T006-044** Cross-artifact audit: every FR/NFR and canonical T060–T069 requirement maps to implementation, tests and evidence; clarification/analysis remain consistent; remove temporary qualification scaffolding.
