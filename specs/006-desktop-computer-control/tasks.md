...
- [x] **T006-P003** [P] Mine `TheHalfMoon/Golam-research@a9f633e09d49a85829b8236331b9e21f7e612634` as untrusted reference evidence; record no admissible donor desktop-control implementation and no donor architecture authority.
...
- [ ] **T006-003** Add pure prepared semantic-action, raw-fallback, capture and clipboard intents with distinct authority classes.
- [ ] **T006-004** Add a platform-neutral `DesktopBackend` trait and deterministic fake backend.
- [ ] **T006-005** Add fake-backend contract tests for observation bounds, permission loss, focus race, stale target, semantic action, capture, fallback and clipboard denial.

## Phase B — Authority, effect and evidence lifecycle

- [ ] **T006-006** Add Kernel prepare/revalidate/dispatch/finalize lifecycle for semantic actions.
- [ ] **T006-007** Add distinct Kernel/Effect lifecycle for raw input fallback with explicit policy/approval requirement.
- [ ] **T006-008** Add bounded capture lifecycle with ephemeral payload default and metadata/digest evidence.
- [ ] **T006-009** Add explicit clipboard read/write lifecycle; deny silent/background inspection.
- [ ] **T006-010** Add durable secret-safe desktop-control evidence, terminal status and `UNKNOWN_OUTCOME` reconciliation.
- [ ] **T006-011** Add restart reconciliation for nonterminal desktop effects/sessions without widening authority.
...
- [ ] **T006-020** Implement selected-source capture contract and resource bounds.
...
- [ ] **T006-024** Prove capture bytes are ephemeral by default, excluded from ordinary logs/evidence, and cannot authorize actions.
...
- [ ] **T006-032** Implement explicit clipboard read/write adapters with permission/capability gating and no polling.
...
- [ ] **T006-038** Add stale window reuse, process restart, focus theft, permission revocation, portal-session termination and target substitution adversarial tests.
...
- [ ] **T006-041** E2E: explicitly select window/display → bounded capture → ephemeral local consumption → release resources.
...
- [ ] **T006-043** E2E: permission/focus/identity drift between prepare and execute fails closed without side effect.
