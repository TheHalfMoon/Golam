...
- [ ] **T006-038** Add stronger-route-still-available, stale/missing fallback eligibility, unreconciled `UNKNOWN_OUTCOME`, absent/stale/mismatched/substituted Kernel/Effect Gate authorization for raw fallback and clipboard, stale window reuse, process restart, focus theft, wrong-window substitution, permission revocation, Windows locked/UAC/secure-desktop/session transitions, capture permission loss after prepare, uncertain completion, portal-session termination, target substitution, visible-control-channel loss, human pause/stop/takeover, stale lease generation and takeover-latency adversarial tests; prove weaker escalation/invisible autonomous actuation is suspended, Gate-invalid dispatch is rejected, and `UNKNOWN_OUTCOME` blocks conflicting retry until reconciliation.
