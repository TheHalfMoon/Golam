...
Build a Rust-owned desktop-control subsystem that preserves the constitutional control-route order, plus a least-privilege Tauri 2 client boundary. The common contract must model route eligibility, observation, target identity, permission/session state, bounded capture, semantic action, explicit raw-input fallback, bounded untrusted pixel hints, clipboard operations, persistent visible autonomous-control state and immediate human pause/stop/takeover. Native adapters are admitted only after fake-backend contract qualification, Source Foundry admission for any introduced dependency/runtime primitive, and platform-specific permission/race testing.

## Technical context

- Trusted core/runtime: Rust workspace (`golam-core`, `golam-kernel`, `golam-ledger`, `golamd`).
- Control-route ordering: `domain/application API → native OS automation API → accessibility/semantic tree → browser DOM/protocol → deterministic keyboard/mouse control → vision/pixel fallback`. Spec 006 owns desktop routes but must not use its weaker fallback paths to bypass a stronger applicable authorized route elsewhere in Golam.
...
- Persistence: immutable request/effect/intent/authority/fallback-eligibility/lease-generation/visible-channel-state bindings, terminal status, reconciliation metadata and payload digests only by default; raw captures/clipboard payloads are not ordinary durable evidence.
...
- **Control-route ordering**: PASS — weaker raw/vision paths require canonical fallback-eligibility evidence and cannot bypass an applicable stronger route.
...
- **Evidence integrity**: PASS — prepared intents and outcomes bind canonical request, effect, target, authority, fallback-eligibility, lease-generation and visible-channel state.
...
1. **Pure contracts** in `golam-core`: route/fallback eligibility evidence, identities, observations, action/capture/clipboard intents, bounded `PixelTargetHint`, `DesktopControlLeaseState`, `VisibleControlChannelState`, immutable request/effect bindings, capability descriptions, errors and canonical digests.
2. **Authority orchestration** in `golam-kernel`: evaluate/select the highest applicable authorized route, prepare/revalidate/dispatch/finalize lifecycle bound to capability/policy/approval/effect records, fallback-eligibility evidence, current control-lease generation and qualified visible-control-channel state; protected human pause/stop/takeover invalidates conflicting agent input authority.
3. **Durable evidence** in `golam-ledger`: route dispositions, prepared request/effect/intent/lease/visible-channel bindings, terminal outcome, permission/session observation and reconciliation metadata without raw sensitive payloads.
4. **Adapter interface** in `golamd`: fake backend plus Windows/macOS/Linux implementations behind a common trait. Adapters report capabilities/results; they cannot decide they are entitled to bypass a stronger route.
...
For any proposed interactive action, the orchestrator records the applicable route sequence and dispositions in canonical `FallbackEligibilityEvidence`.

- Select the highest applicable route that remains available and authorized.
- A route may be skipped only when it is inapplicable, unavailable, unsupported, denied by required authority/permission, or failed safely before any ambiguous side effect.
- `UNKNOWN_OUTCOME` never authorizes escalation to a weaker conflicting route; reconcile first.
- Raw adapters and pixel-hint producers cannot self-mint fallback eligibility.
- Domain/app and browser routes may live outside Spec 006, but their applicability must still be respected before Spec 006 chooses deterministic raw or vision/pixel fallback.

## Action lifecycle

`evaluate routes → select highest eligible route → observe/select target → create ToolRequest → prepare immutable intent bound to fallback eligibility when applicable + current control-lease generation + qualified visible-control-channel state → capability/policy/approval → Effect PREPARED → Kernel/Effect Gate → immediate route/identity/permission/binding/lease-generation/visible-channel revalidation → platform dispatch → post-action observation → Effect terminal/reconciliation`

If execution may have crossed the side-effect boundary and terminal truth is uncertain, the result is `UNKNOWN_OUTCOME` and blocks conflicting follow-up or fallback escalation until reconciliation.
...
- the hint cannot contain or manufacture an OS handle, capability, approval, semantic identity or fallback eligibility;
