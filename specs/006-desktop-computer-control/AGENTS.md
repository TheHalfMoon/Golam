...
- A weaker route may run only with canonical fallback-eligibility evidence showing every applicable stronger route is inapplicable, unavailable, unsupported, authority/permission denied, or safely failed before any ambiguous side effect. `UNKNOWN_OUTCOME` blocks conflicting weaker escalation until reconciliation. Raw adapters and pixel-hint producers cannot self-mint eligibility.
- Semantic desktop APIs are primary within Spec 006. Raw coordinate/input fallback is explicit-only.
- A bounded vision/pixel fallback may produce only an untrusted candidate region/coordinate from an explicitly selected capture. It cannot mint semantic identity, capability, approval, fallback eligibility or action authority. OCR/text extraction from raw screenshot pixels remains deferred to Spec 007.
...
- Every side-effect-capable or privacy-sensitive operation must bind an exact ToolRequest/request digest, immutable Effect/effect binding digest and canonical intent digest, then traverse capability → policy → approval where required → Effect PREPARED → Kernel/Effect Gate → immediate route/binding/target/session/permission/control-lease/visible-channel revalidation → bounded platform adapter → terminal evidence/reconciliation.
- Missing, mismatched, stale or substituted request/effect/intent/authority/fallback-eligibility/target/session/control-lease state fails closed before adapter dispatch.
- If a side effect may have crossed the effect boundary but terminal truth is uncertain, record `UNKNOWN_OUTCOME`; restart, timeout, permission transition, human takeover or adapter failure cannot authorize a conflicting retry or weaker fallback before reconciliation.
...
- Prepared actions must fail closed when route eligibility, work-surface, focused element, permission, session, capability, policy, approval, platform identity, control-lease generation or visible-control-channel state drifts.
...
- Rust owns trusted route selection/fallback eligibility, desktop-control contracts, authenticated client handling, authority checks, control-lease/takeover state, visible-control-channel safety state, adapter boundaries, evidence and lifecycle state.
...
- Route-order qualification must cover stronger-route-still-available, stale/missing eligibility evidence, and unreconciled `UNKNOWN_OUTCOME` blocking weaker escalation.
