...
A bounded vision/pixel fallback may supply only an untrusted candidate region/coordinate derived from an explicitly selected capture. It cannot mint authority, cannot bypass exact work-surface/focus/session identity, and cannot perform OCR/text extraction under Spec 006.

**Acceptance**: no background keylogging, no arbitrary global injection, no secure-desktop interaction, no silent fallback, and no pixel-derived candidate can execute without a separately authorized raw-input effect.
...
- **FR-002**: Define canonical `WorkSurfaceIdentity`, `SemanticElementIdentity`, `DesktopObservation`, `PreparedDesktopAction`, `CaptureIntent`, `PixelTargetHint`, `DesktopControlLeaseState`, `DesktopActionOutcome` and permission/session evidence.
