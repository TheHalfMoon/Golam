    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidCredentialLength { actual } => write!(
                f,
                "client credential has {actual} bytes; expected {CREDENTIAL_BYTES}"
            ),
            Self::InvalidCredentialMagic => f.write_str("client credential magic is invalid"),
            Self::UnsupportedCredentialVersion(version) => {
                write!(f, "client credential version {version} is unsupported")
            }
            Self::CredentialClientMismatch => {
                f.write_str("client credential belongs to a different client id")
            }
            Self::CredentialKeyMismatch => {
                f.write_str("client credential key fingerprint does not match")
            }
            Self::CredentialPublicKeyMismatch => {
                f.write_str("client credential public key does not match derived key")
            }
        }
    }
}
...
impl From<getrandom::Error> for CredentialError {
    fn from(value: getrandom::Error) -> Self {
        Self::Random(value)
    }
}
impl From<AuthorityPathError> for CredentialError {
    fn from(value: AuthorityPathError) -> Self {
        Self::AuthorityPath(value)
    }
}

pub struct ClientCredentialStore<'a> {
    authority: &'a AuthorityLayout,
}
impl<'a> ClientCredentialStore<'a> {
    pub const fn new(authority: &'a AuthorityLayout) -> Self {
        Self { authority }
    }
    ...
    pub fn generate(
        ...
        if client_id.0 == 0 {
            return Err(CredentialError::InvalidClientId);
        }
    ...
    pub fn load(
        ...
        client_id: ClientId,
        key_id: ClientKeyId,
    ) -> Result<SigningKey, CredentialError> {
        let path = self.authority.credential_path(client_id.0, &key_id.0)?;
...
fn decode_credential(
    ...
    expected_key_id: ClientKeyId,
) -> Result<SigningKey, CredentialError> {
    if bytes.len() != CREDENTIAL_BYTES {
        return Err(CredentialError::InvalidCredentialLength {
            actual: bytes.len(),
        });
    }
    if bytes[..4] != CREDENTIAL_MAGIC {
        return Err(CredentialError::InvalidCredentialMagic);
    }
    let version = u16::from_be_bytes([bytes[4], bytes[5]]);
    if version != CREDENTIAL_VERSION {
        return Err(CredentialError::UnsupportedCredentialVersion(version));
    }
    let stored_client = ClientId(u128::from_be_bytes(
        bytes[6..22]
            .try_into()
            .expect("fixed credential client-id range"),
    ));
    if stored_client != expected_client {
        return Err(CredentialError::CredentialClientMismatch);
    }
    let stored_key_id = ClientKeyId(
        bytes[22..54]
            .try_into()
            .expect("fixed credential key-id range"),
    );
    if stored_key_id != expected_key_id {
        return Err(CredentialError::CredentialKeyMismatch);
    }
    let mut secret: [u8; 32] = bytes[54..86]
        .try_into()
        .expect("fixed credential secret range");
    let stored_public: [u8; 32] = bytes[86..118]
        .try_into()
        .expect("fixed credential public-key range");
    let signing_key = SigningKey::from_bytes(&secret);
    secret.fill(0);
    let derived_public = signing_key.verifying_key().to_bytes();
    if derived_public != stored_public {
        return Err(CredentialError::CredentialPublicKeyMismatch);
    }
    if key_id_for_public_key(derived_public) != expected_key_id {
        return Err(CredentialError::CredentialKeyMismatch);
    }
    Ok(signing_key)
}
...
mod tests {
    ...
    fn authority() -> (RuntimeLayout, AuthorityLayout) {
        ...
        let runtime = RuntimeLayout::initialize(
            std::env::temp_dir().join(format!("golam-credential-{}-{t}-{n}", std::process::id())),
        )
        .unwrap();
    ...
    fn generated_key_round_trips() {
        ...
        signing
            .verifying_key()
            .verify(b"golam-credential-proof", &signature)
            .unwrap();
        store.remove(generated.client_id, generated.key_id).unwrap();
    ...
    fn corruption_fails_closed() {
        ...
        let mut bytes = fs::read(&generated.path).unwrap();
        bytes[117] ^= 0xff;
        fs::write(&generated.path, &bytes).unwrap();
