...
const LEASE_ISSUE_INTENT_DOMAIN: &[u8] = b"golam:capability-lease-issue-intent:v1";
const LEASE_ID_DOMAIN: &[u8] = b"golam:capability-lease-id:v1";
const LEASE_AUTHORITY_DOMAIN: &[u8] = b"golam:capability-lease-authority:v1";
const LEASE_REVOCATION_INTENT_DOMAIN: &[u8] = b"golam:capability-lease-revocation-intent:v1";
const LEASE_REVOCATION_ID_DOMAIN: &[u8] = b"golam:capability-lease-revocation-id:v1";
const LEASE_APPROVAL_CONSUMPTION_DOMAIN: &[u8] = b"golam:capability-lease-approval-consumption:v1";
...
impl CapabilityLeaseRecord {
    pub const fn binding(&self) -> CapabilityLeaseBinding {
        CapabilityLeaseBinding::new(self.lease_id, self.generation, self.authority_digest)
    }
}
...
impl PreparedCapabilityLeaseIssue {
    ...
    pub const fn parent(&self) -> Option<CapabilityLeaseBinding> {
        self.parent
    }
}
...
impl PreparedCapabilityLeaseRevocation {
    ...
    pub const fn lease(&self) -> CapabilityLeaseBinding {
        self.lease
    }
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ApprovalMismatch => {
                f.write_str("capability lease mutation approval does not match exact effect/action/resource")
            }
...
pub fn prepare_capability_lease_issue(
    ...
    encoder.push_bytes(LEASE_ISSUE_INTENT_DOMAIN)?;
...
pub fn prepare_capability_lease_revocation(
    ...
    let mut encoder = CanonicalEncoder::new();
    encoder.push_bytes(LEASE_REVOCATION_INTENT_DOMAIN)?;
...
fn verify_parent_lease(
    ...
    let authority_digest: [u8; 32] = row
        .8
        .try_into()
        .map_err(|_| CapabilityLeaseMutationError::InvalidStoredRecord("parent authority digest is not 32 bytes"))?;
...
fn verify_revocation_target(
    ...
    let authority_digest: [u8; 32] = row
        .2
        .try_into()
        .map_err(|_| CapabilityLeaseMutationError::InvalidStoredRecord("lease authority digest is not 32 bytes"))?;
...
fn issue_authority_digest(
    ...
    let mut encoder = CanonicalEncoder::new();
    encoder.push_bytes(LEASE_AUTHORITY_DOMAIN)?;
...
fn approval_consumption_id(approval_id: [u8; 16], effect_id: EffectId) -> [u8; 16] {
    let mut hasher = blake3::Hasher::new();
    hasher.update(LEASE_APPROVAL_CONSUMPTION_DOMAIN);
    hasher.update(&approval_id);
    hasher.update(&effect_id.0.to_be_bytes());
    let digest = hasher.finalize();
    let mut id = [0_u8; 16];
...
fn seq_from_i64(value: i64) -> Result<u64, CapabilityLeaseMutationError> {
    u64::try_from(value).map_err(|_| CapabilityLeaseMutationError::InvalidStoredRecord("negative sequence"))
}
...
fn load_state(
    ...
    row.map(|row| {
        let parent_lease_id = row.1.map(runtime_id_from_vec).transpose()?;
        ...
        let authority_digest = runtime_hash_from_vec(row.6)?;
...
fn runtime_id_from_vec(value: Vec<u8>) -> Result<[u8; 16], CapabilityLeaseRuntimeError> {
    value
        .try_into()
        .map_err(|_| CapabilityLeaseRuntimeError::InvalidStoredRecord("lease id is not 16 bytes"))
}

fn runtime_hash_from_vec(value: Vec<u8>) -> Result<[u8; 32], CapabilityLeaseRuntimeError> {
...
mod tests {
    ...
    fn authority() -> (RuntimeLayout, AuthorityLayout) {
        ...
        let runtime = RuntimeLayout::initialize(std::env::temp_dir().join(format!(
            "golam-capability-lease-mutation-{}-{t}-{n}",
            std::process::id()
        )))
        .unwrap();
    ...
    #[test]
    fn issue_and_revoke_are_atomic_protected_mutations_with_integrity_evidence() {
        ...
        let prepared = prepare_capability_lease_issue(
            ...
            &["session:1".to_owned()],
            &["local-session:1".to_owned()],
            Some("2026-08-27T00:00:00Z"),
            Some("2026-08-28T00:00:00Z"),
        )
        .unwrap();
        let effect_id = EffectId(1000);
        let approval_id = [10_u8; 16];
        ...
        let mut store = CapabilityLeaseStore::open(&authority).unwrap();
        let issued = store
        ...
        assert_eq!(chain[0].principal_id, "client:9:alice");

        let prepared_revoke = prepare_capability_lease_revocation(
            issued.binding(),
            "manual_revoke",
            "2026-08-27T12:00:00Z",
        )
        .unwrap();
        let revoke_effect = EffectId(1200);
        let revoke_approval = [12_u8; 16];
        let revoke_decision = authorize_mutation(
            &authority,
            revoke_effect,
            revoke_approval,
            CAPABILITY_LEASE_REVOKE_ACTION,
            prepared_revoke.resource(),
            prepared_revoke.intent_digest(),
            1300,
        );
        let mut store = CapabilityLeaseStore::open(&authority).unwrap();
        let revoked = store
            ...
            .unwrap();
        assert_eq!(revoked.lease_id, issued.lease_id);
        drop(store);
    ...
    #[test]
    fn direct_store_self_grant_fails_even_with_allow_effect_and_approval() {
        let (runtime, authority) = authority();
        let prepared = prepare_capability_lease_issue(
            "owner:owner",
            None,
        ...
        .unwrap();
        let effect_id = EffectId(2000);
        let approval_id = [20_u8; 16];
        let decision = authorize_mutation(
            &authority,
            effect_id,
            approval_id,
        ...
        let mut store = CapabilityLeaseStore::open(&authority).unwrap();
        assert!(matches!(
            store.issue(prepared, decision.decision_id, approval_id, effect_id),
    ...
    #[test]
    fn child_issue_rejects_scope_or_lifetime_widening_at_store_boundary() {
        let (runtime, authority) = authority();
        let parent_prepared = prepare_capability_lease_issue(
            "client:9:alice",
            None,
            &["session.read".to_owned()],
            &["session:1".to_owned()],
        ...
        .unwrap();
        let parent_effect = EffectId(3000);
        let parent_approval = [30_u8; 16];
        let parent_decision = authorize_mutation(
            &authority,
            parent_effect,
            parent_approval,
            CAPABILITY_LEASE_ISSUE_ACTION,
            parent_prepared.resource(),
            parent_prepared.intent_digest(),
            3100,
        );
        let mut store = CapabilityLeaseStore::open(&authority).unwrap();
        let parent = store
        ...
        let child_prepared = prepare_capability_lease_issue(
            "client:9:alice",
            Some(parent.binding()),
        ...
        .unwrap();
        let child_effect = EffectId(3200);
        let child_approval = [32_u8; 16];
        let child_decision = authorize_mutation(
            &authority,
            child_effect,
            child_approval,
            CAPABILITY_LEASE_ISSUE_ACTION,
            child_prepared.resource(),
            child_prepared.intent_digest(),
            3300,
        );
        let mut store = CapabilityLeaseStore::open(&authority).unwrap();
        assert!(matches!(
            store.issue(
                child_prepared,
                child_decision.decision_id,
                child_approval,
                child_effect,
            ),
            Err(CapabilityLeaseMutationError::ParentScopeWidening)
                | Err(CapabilityLeaseMutationError::ParentTemporalWidening)
        ));
        drop(store);
        fs::remove_dir_all(runtime.root).unwrap();
    }
